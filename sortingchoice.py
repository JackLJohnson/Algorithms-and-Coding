...
- Plot the time taken by each sorting algorithm by varying "n"
'''
...
def merge(A, p, q, r):
    ...
    #print("values of i, j , counter_array, p , r", i, j, counter_array, p, r)
    if (i<n1):
        for k in range(0, n1-i):
            A[counter_array+k+p] = L[i+k]

    if (j<n2):
        for l in range(0, n2-j):
            A[counter_array+l+p] = R[j+l]
...
class efficientsorting(object):
    ...
    def __init__(self):
        ...
        #TODO: Have a variable list of sizes and then plot of times
        ...
        '''
        - One approach is to get the choice , the other is to just see what is the time taken for each case at one go
        '''
